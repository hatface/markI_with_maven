...
public class SplashFrame extends JWindow implements Runnable {  
    ...
    public SplashFrame(MainFrame mainframe) throws MalformedURLException {  
    	this.mainframe = mainframe;
        Container container = getContentPane(); // 得到容器  
        setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR)); // 设置光标  
//        URL url = this.getClass().getClassLoader().getResource(); // 图片的位置  
//        System.out.println(url.getp);
//        if (url != null) {  
            container.add(new JLabel(new ImageIcon("resources/images/loading.gif")), BorderLayout.CENTER); // 增加图片  
//        }  
