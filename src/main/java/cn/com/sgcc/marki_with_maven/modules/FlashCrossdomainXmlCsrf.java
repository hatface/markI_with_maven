...
public class FlashCrossdomainXmlCsrf implements IPocBase {
	...
	public Map info() {
		...
		info.put("name", "FlashCrossdomainXmlCsrf");
		return info;
	}
	...
	public boolean verify(Map infodict) {
		...
		try {
			...
			if (resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK)
			{
				...
				Element  selectObject = (Element) parseText.selectObject("allow-access-from/domain");
				if(selectObject.getStringValue().equals("*"))
				{
					success = true;
				}
			}
